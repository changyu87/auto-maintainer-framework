...
| 5 | `scheduling` | §3.3.1–§3.3.4 | In-session heartbeat + script-backed `/auto-maintainer:start`/`:stop`/`:status`; route now **GUARD→DRAIN→PULL→PERSIST→EXIT** (read-and-idle, real work-intake PULL). Route is **data** (default + project-local `route.json` override via adapter-wiring §3.4.3). 1-min hardcoded (#17); system-cron + durable heartbeat deferred (#31); `/start` clears a latched STOPPED (#44). | **implemented** (PRs #21/#25/#33/#41/#46/#56; 56 tests) |
...
| 12 | `packaging-config` | §3.4.3, §3.10.1, §3.10.2, §3.10.4, §3.10.5 | **Slices 1–2:** clean plugin assembly (no `.rabbit/`) + `marketplace.json` + `ship/` collection + 5 loop libs + `/auto-maintainer:start`/`:stop`/`:status` + SessionStart persona. Later: `userConfig`, port→adapter wiring, dogfood. | **slice 2 implemented** (PRs #13/#23/#27/#34/#42/#47/#57; **v0.2.5**; 24 tests; ships route-as-data loop, 10 libs) |
