...
| 5 | `scheduling` | §3.3.1–§3.3.4 | In-session heartbeat + script-backed `/auto-maintainer:start`/`:stop`/`:status`; route now **GUARD→DRAIN→PULL→PERSIST→EXIT** (read-and-idle, real work-intake PULL). 1-min hardcoded (#17); system-cron + durable heartbeat deferred (#31). | **implemented** (PRs #21/#25/#33/#41; 31 tests) |
| 6 | `work-intake` | §3.4.2 (PULL), §3.5.1–§3.5.3, §3.5.5, §3.5.7, §3.5.8 | **Slice 1 (PULL):** GitHub-Issues PULL → `work_items` (gh CLI behind injectable seam). **Slice 2:** TRIAGE pipeline (normalize, validity, dedup-vs-closed, 1-level decompose, ordering, WHAT-gen seam) → `work_orders`. | **slice 1 implemented + wired live** (PRs #38/#41; 10 tests; loop pulls real issues) |
...
| 12 | `packaging-config` | §3.4.3, §3.10.1, §3.10.2, §3.10.4, §3.10.5 | **Slices 1–2:** clean plugin assembly (no `.rabbit/`) + `marketplace.json` + `ship/` collection + 5 loop libs + `/auto-maintainer:start`/`:stop`/`:status` + SessionStart persona. Later: `userConfig`, port→adapter wiring, dogfood. | **slice 2 implemented** (PRs #13/#23/#27/#34/#42; **v0.2.3**; 21 tests) |
