...
SessionStart hook on the next session (DESIGN §3.3.2 heartbeat bootstrap).

Two durable facts live in the runtime dir, alongside the disposition + lock
...
**The dedup is owned by the SessionStart path, NOT by /start.**
``record_loop_intent`` (the ``/start`` path) does NOT touch the resume-dedup. The
SessionStart hook is what tells the session to run ``/start``; if ``/start``
cleared the dedup, a *second* SessionStart in the SAME session (SessionStart
fires on startup / resume / clear / compact) would re-arm a DUPLICATE heartbeat.
So only ``mark_resumed`` (set by the hook on a True decision) and
``clear_loop_intent`` (the ``/stop`` path, which ends the arming epoch) ever
write/clear the dedup — ``/start`` leaves it exactly as the hook left it. A
manual ``/start`` with no prior arming simply finds no dedup stamp and arms on
the next SessionStart as expected.
...
def record_loop_intent(runtime_dir):
    ...
    It deliberately does NOT touch the resume-dedup. The SessionStart auto-resume
    hook is what asks the session to run ``/start``; if ``/start`` cleared the
    dedup, a second SessionStart in the SAME session would re-arm a DUPLICATE
    heartbeat. Dedup ownership lives entirely on the SessionStart path
    (``mark_resumed``) and the ``/stop`` path (``clear_loop_intent``)."""
...
def clear_loop_intent(runtime_dir):
    ...
    A stop ends the current arming epoch, so it ALSO clears the resume-dedup:
    that way a future ``/start`` in this same session (a human resume without a
    restart) can be re-armed by the next SessionStart instead of being silenced
    by a stale dedup stamp."""
    try:
...
def should_auto_resume(runtime_dir, session_id):
    ...
    NOTE on RESTART_NEEDED (DESIGN §3.3.4): a latched RESTART_NEEDED BLOCKS
    auto-resume here (the safe, conservative choice — the loop is not silently
    re-armed). This does NOT implement the §3.3.4 RESTART_NEEDED→SessionStart
    auto-resume flow (which would *drive* a resume after a restart); that remains
    deferred. The gate only guarantees a restart-owed loop is never re-armed
