...
| Command | Description |
| --- | --- |
| `/auto-maintainer:adapter-map` | View and edit the loop's adapter map — which adapter implements each route port (GUARD, DRAIN, PULL, TRIAGE, PRIORITIZE, IMPLEMENT, VERIFY, INTEGRATE, CLEANUP, PERSIST, EXIT). Every edit is validated before it is saved. |
