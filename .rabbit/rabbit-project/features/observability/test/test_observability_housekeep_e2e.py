#!/usr/bin/env python3
"""End-to-end housekeeping conformance for the observability doc-slim wave.

This wave is a MEASURED doc reduction (rabbit-housekeep): docs/spec.md is
slimmed of redundant prose (the Purpose section's escalation-vs-REPORT
distinction, which is restated verbatim by the Invariants and Deferred sections
that own it) without losing any load-bearing claim. docs/contract.md is already
minimal and is unchanged. These tests are the deterministic gate on that wave —
they are e2e in that they read the SHIPPED doc artifacts (not a mock) and run
the SHIPPED behaviour suite, asserting the wave's three contractual properties
end to end:

  Gate 0 — BEHAVIOR PRESERVED (the one MANDATORY gate). The existing
  observability behaviour suite (test_observability_e2e.py, the 10 tests incl.
  the escalation provenance + sink-failure tests and the event-log seq
  round-trip) still passes after the slim. A doc edit that broke the feature
  suite FAILS. This gate runs that module in-process so it never recurses into
  this housekeep module.
  ...
  symbol, an event-log schema field, a closed-vocab kind, the escalation
  provenance stamp, a live cross-feature reference, and the contract
...
_BEHAVIOUR_SUITE = os.path.join(_TEST_DIR, "test_observability_e2e.py")

# measure-reduction.py is the script-tier line-accounting tool (rabbit-housekeep
# scripts dir). This test lives at
# <repo>/rabbit-project/features/observability/test/, so walk up to the repo
...
# doc surface (spec.md + contract.md): the public-surface symbols, the
# event-log schema fields, two closed-vocab kinds, the escalation provenance
# stamp, and the live cross-feature references live in the spec; the
...
_LOAD_BEARING_DOCS = (
    ...
    "kind",
    "detail",
    # Closed-vocab kinds (anchors of the closed set).
    "tick_start",
    "escalation",
    # Escalation provenance stamp.
    "filed_by: autonomous-maintainer",
    # Live cross-feature references (safety-governance + work-intake exist; the
    # outbound-report ref is the load-bearing deferred-destination intent).
    "safety-governance",
    "work-intake",
    "outbound-report",
)
...
def _load_behaviour_module(alias):
    """Import the behaviour module (test_observability_e2e.py) under a private
...
def test_behaviour_suite_still_green():
    """The pre-existing observability behaviour suite (10 tests incl. the
    escalation provenance/sink-failure tests and the event-log seq round-trip)
    still passes after the doc slim. Imported and run in-process so this
    housekeep module is NOT discovered (no recursion); a raised assertion fails
    this gate loudly."""
    module = _load_behaviour_module("observability_behaviour_suite")
    fns = sorted(
        n for n in dir(module)
        if n.startswith("test_") and callable(getattr(module, n)))
    assert len(fns) == 10, (
        f"expected the 10-test behaviour suite, found {len(fns)}: {fns}")
    for fn_name in fns:
        getattr(module, fn_name)()


def test_behaviour_suite_has_ten_tests_including_escalation():
    """The behaviour suite count is exactly 10 and includes the escalation
    provenance test the spec names (the escalation channel is half the slice)."""
    module = _load_behaviour_module("observability_behaviour_suite_count")
    fns = sorted(
        n for n in dir(module)
        if n.startswith("test_") and callable(getattr(module, n)))
    assert len(fns) == 10, f"expected 10 behaviour tests, found {len(fns)}: {fns}"
    assert any("escalate" in n for n in fns), (
        "the escalation behaviour tests must be present")
...
def test_spec_retains_design_citations():
    """The DESIGN §-citations are load-bearing provenance (coding-rules §6: a
    citation is load-bearing) and MUST survive the slim — this wave touches only
    redundant prose, never a DESIGN citation."""
    text = _read(_SPEC)
    for cite in ("§3.9", "§3.9.1", "§3.9.3", "§1.3", "§3.11"):
        assert cite in text, f"DESIGN citation dropped from spec.md: {cite}"
