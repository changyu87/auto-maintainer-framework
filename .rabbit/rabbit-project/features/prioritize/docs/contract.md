...
```json
{
  "provides": {
    "files": [
      ...
      "same-feature serialization (#214, #257, #258): at most one accepted order per target feature per tick (FIFO-first wins, the rest deferred); cross-feature orders stay parallel. Target feature(s) read from the order's authoritative target_feature field (TRIAGE-stamped, #258); when that field is absent PRIORITIZE falls back to re-deriving from feature:/component: prefixed labels, a Component: body line, a conventional title prefix (name: / type(scope):, for label-less issues), and a [scope@team]/[scope] bracket-prefix title (a live-pool filing convention; the scope before any @team is the key) only (never generic labels, never a bare conventional-commit type, never the 'and' connector)"
    ],
    "scripts": [],
    "skills": []
  },
