#!/usr/bin/env python3
"""End-to-end housekeeping conformance for the tick-orchestrator doc-slim wave.
...
"## Current behaviour" section that re-enumerated the Public surface) without
losing any load-bearing claim. docs/contract.md is already minimal and is
unchanged. These tests are the deterministic gate on that wave — they are e2e in
that they read the SHIPPED doc artifacts (not a mock) and run the SHIPPED
behaviour suite, asserting the wave's three contractual properties end to end:

  Gate 0 — BEHAVIOR PRESERVED (the one MANDATORY gate). The existing
  tick-orchestrator behaviour suite (test_orchestrator_e2e.py, the 13 tests incl.
  the PING/PONG run-to-terminal conformance and the decoupling insert-state
  proof) still passes after the slim. A doc edit that broke the feature suite
  FAILS. This gate imports that module in-process under a private alias so it
  never recurses into this housekeep module.
  ...
  (resolve_next, the run loop, the structural validators), the route schema, the
  consumed fsm-contracts types, the decoupling guarantee, the ping-pong
  conformance symbols, and the contract provides/reads/invokes/never block keys
  MUST still appear in the slimmed docs. A slim that drops a load-bearing token
  FAILS.
...
_BEHAVIOUR_SUITE = os.path.join(_TEST_DIR, "test_orchestrator_e2e.py")

# measure-reduction.py is the script-tier line-accounting tool (rabbit-housekeep
# scripts dir). This test lives at
# <repo>/rabbit-project/features/tick-orchestrator/test/, so walk up to the repo
...
# doc surface (spec.md + contract.md): the public-surface symbols, the route
# schema, the consumed fsm-contracts types, the decoupling guarantee, and the
# ping-pong conformance symbols live in the spec; the
...
def _load_behaviour_module(alias):
    """Import the behaviour module (test_orchestrator_e2e.py) under a private
...
def test_behaviour_suite_still_green():
    """The pre-existing tick-orchestrator behaviour suite (13 tests incl. the
    PING/PONG run-to-terminal conformance and the decoupling insert-state proof)
    ...
    this gate loudly."""
    module = _load_behaviour_module("orchestrator_behaviour_suite")
    fns = sorted(
        n for n in dir(module)
        if n.startswith("test_") and callable(getattr(module, n)))
    assert len(fns) == 13, (
        f"expected the 13-test behaviour suite, found {len(fns)}: {fns}")
    for fn_name in fns:
        getattr(module, fn_name)()


def test_behaviour_suite_has_thirteen_tests_including_conformance():
    """The behaviour suite count is exactly 13 and includes the headline
    PING/PONG run-to-terminal conformance test the spec names."""
    module = _load_behaviour_module("orchestrator_behaviour_suite_count")
    fns = sorted(
        n for n in dir(module)
        if n.startswith("test_") and callable(getattr(module, n)))
    assert len(fns) == 13, f"expected 13 behaviour tests, found {len(fns)}: {fns}"
    assert any("pingpong_run_to_terminal" in n for n in fns), (
        "the e2e PING/PONG run-to-terminal conformance test must be present")
