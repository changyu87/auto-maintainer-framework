...
# ---------------------------------------------------------------------------
def test_version_bumped_to_0_20_0_and_consistent():
    out_root = _build_into_temp()
    try:
        pj = os.path.join(
            out_root, "plugins", "auto-maintainer",
        ...
        with open(mk, encoding="utf-8") as fh:
            mdata = json.load(fh)
        assert pdata.get("version") == "0.20.0", \
            f"plugin.json version must be 0.20.0, got {pdata.get('version')!r}"
        assert mdata["plugins"][0].get("version") == "0.20.0", \
            "marketplace.json plugin entry version must be 0.20.0"
