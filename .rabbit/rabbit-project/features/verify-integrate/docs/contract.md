...
```json
{
  "provides": {
    "files": [
      ...
      "feature_run_py_path() + default_complement_runner(): the deterministic injectable feature-run.py resolver + the self-contained complement-runner (shells a named feature's test/run.py; modeled on FT-A test_gate.py, NOT an import of implement)",
