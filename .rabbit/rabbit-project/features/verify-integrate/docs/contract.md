...
```json
{
  ...
  "reads": {
    ...
    "external": [
      "gh CLI — pr list (the loop's open auto-maintainer-labelled PRs; --json closingIssuesReferences for RECONCILE dedup C), pr view --json mergeable (VERIFY's bounded transient-UNKNOWN mergeability poll), pr checks (CI rollup), pr merge --merge --delete-branch, pr close --delete-branch (RECONCILE same-issue dedup)"
    ]
  },
