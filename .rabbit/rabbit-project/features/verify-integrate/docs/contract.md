...
```json
{
  "provides": {
    "files": [
      ...
      "GATE doc-surface load-bearing-token survival check (issue #353): declared_load_bearing_tokens(feature_dir)/missing_load_bearing_tokens(feature_dir)/features_with_changed_doc_surfaces(changed_paths, features_root) + GateResult reason 'load-bearing'. A feature declares its must-survive tokens in test/load_bearing_tokens.json ({tokens:[...]}); GATE fails (rolls back + excludes) a doc-touched PR that drops a declared token from its post-change doc surfaces (docs/spec.md, docs/contract.md, skills/*/SKILL.md). Opt-in per feature + doc-scoped: no declaration or no doc-surface change = unaffected",
