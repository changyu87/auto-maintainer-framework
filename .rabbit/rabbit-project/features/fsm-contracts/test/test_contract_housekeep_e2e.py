#!/usr/bin/env python3
"""End-to-end housekeeping conformance for the fsm-contracts doc-slim wave.

This wave is a MEASURED doc reduction (rabbit-housekeep): docs/spec.md is
slimmed of dead/redundant prose (a proven-dead docs/ROADMAP.md reference and an
implementation-history section) without losing any load-bearing claim.
...
doc artifacts (not a mock) and assert the wave's two contractual properties end
to end:
  ...
  the route schema, the validator, the uniform signature, the closed signal
  vocabulary members, the four core anchors, the ping-pong conformance symbols,
...
# test lives at <repo>/rabbit-project/features/fsm-contracts/test/, so walk up to
...
# Load-bearing tokens that MUST survive the slim. These are asserted against the
# COMBINED doc surface (spec.md + contract.md): the public-surface types and the
# signal vocabulary live in the spec; the provides/reads/invokes/never keys live
# in the contract block. Membership in the union is what "survived the slim"
# means for the wave.
_LOAD_BEARING_DOCS = (
    "TickContext",
    "StateResult",
