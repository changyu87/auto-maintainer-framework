...
(re-clearing every minute would defeat a `/stop` that lands between heartbeats).
...
1. Clear the latch (no tick yet) with the deterministic starter:
   ...
   message and STOP — do not proceed to steps 2 or 3.
...
3. Schedule the recurring heartbeat as a **prompt** (so the session is present to
   ...
   tick can't fulfill agent-state dispatches — the heartbeat must wake the
   session.
