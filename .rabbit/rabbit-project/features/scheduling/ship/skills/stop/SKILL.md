...
1. Latch the loop STOPPED by running the stop script:
   ...
   loop-intent so it does NOT re-arm the heartbeat. Do not hand-roll any Python
   or write the markers yourself — `stop.py` owns the state writes; this skill
   only runs it. Print the confirmation line it emits.
