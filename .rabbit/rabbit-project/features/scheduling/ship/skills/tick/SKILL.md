...
- `{"status":"paused","state":"<name>","dispatches":[ {"subagent_type","prompt",
  ... }, ... ]}` — the runner is waiting at an agent-state. Dispatch each entry,
  then resume.
...
3. If `status` is `"paused"`: for **each** entry in `dispatches` (in order),
   ...
   The prompt is the complete, self-contained handoff contract — pass it through
   **verbatim**, do not author or alter it. Each subagent writes its own output
   file (as the prompt instructs) and replies with a short ack; you do **not**
   write any file yourself and you do **not** need to capture the subagent's
   output — just let each dispatch finish.

4. Resume the runner — it reads the subagent-written output files itself:

   ```
   python3 ${CLAUDE_PLUGIN_ROOT}/lib/run_tick.py --resume
   ```

   (`--resume` takes no arguments; the runner knows the output files from its
   checkpoint.) Parse the JSON it prints and go back to step 2 with the result.
...
- Dispatch a subagent ONLY for the entries the runner hands you, with the prompt
  the runner rendered, verbatim — do not alter the prompt or invoke any other
  subagent. The prompt is the machine-first invocation envelope; it alone tells
  the subagent the schema, the output file, and the ack — the subagent needs no
  other knowledge.
